    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                const profileData = data.memberProfile || {};
        ...
        <div className="max-w-4xl mx-auto px-4 py-16 md:py-24">
            ...
            <div className="bg-white/90 backdrop-blur-sm p-8 md:p-12 rounded-2xl shadow-xl border border-rose-100 transform transition-all hover:scale-[1.01]">
                <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
                    {/* Avatar */}
                    <img
                        src={profile?.photoProfile || "/img/profile/default.jpg"}
                        alt="Profile Photo"
                    ...
                    <div className="flex-grow text-center md:text-left">
                        <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-2">{profile?.name || user?.name}</h2>
                        <p className="text-gray-500 mb-4">{user?.email}</p>

                        <div className="space-y-3 mt-6">
                            <div className="flex items-center gap-3 text-gray-700">
                                <User className="w-5 h-5 text-pink-500" />
                                <span>Full name: {profile?.name || user?.name}</span>
                            </div>
                            <div className="flex items-center gap-3 text-gray-700">
